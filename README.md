...
## Interaction with `ignorePatterns`

A `.base` file that matches Quartz's `configuration.ignorePatterns` is **not emitted as a page**. `ctx.allFiles` is globbed with the static config patterns at load time, but plugins may append patterns during the build (for example a `.pubignore` confidentiality filter that adds patterns in a `shouldPublish` filter). So `generate` re-checks each base against the current `ignorePatterns` — mirroring Quartz core's own `ignored()` (`minimatch` per pattern) — ensuring an ignored base is withheld just like ignored markdown and assets.

## Interaction with `unlisted` pages

`bases-page` respects the `file.data.unlisted` convention for the **entries a base lists**. Pages marked `unlisted: true` (by `@quartz-community/unlisted-pages`, or by `@quartz-community/encrypted-pages` via `unlistWhenEncrypted: true` or per-page `unlisted: true` / `stealth: true`) are excluded from every rendered base view — table, list, board, cards, gallery, and any custom view registered via `viewRegistry`. Both the main entry loop and the internal `fileLookup` used by `.asFile()` method resolution skip unlisted pages, so they cannot be dereferenced from formulas on visible pages either. Pages marked `unlisted: true` (by `@quartz-community/unlisted-pages`, or by `@quartz-community/encrypted-pages` via `unlistWhenEncrypted: true` or per-page `unlisted: true` / `stealth: true`) are excluded from every rendered base view — table, list, board, cards, gallery, and any custom view registered via `viewRegistry`. Both the main entry loop and the internal `fileLookup` used by `.asFile()` method resolution skip unlisted pages, so they cannot be dereferenced from formulas on visible pages either.
