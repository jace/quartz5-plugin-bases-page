...
import { u, evaluate, evaluateFilter, slugifyPath, resolvePropertyValue, S, transformLink } from './chunk-X2AZ5GOJ.js';

createRequire(import.meta.url);

// src/components/ViewSelector.tsx
function viewSlug(view, index) {
...
var bases_inline_default = `function d(e,n){let t=e.querySelectorAll(".bases-view-tabs button"),r=e.querySelectorAll(".bases-view");t.forEach(o=>{o.classList.toggle("is-active",o.dataset.viewIndex===String(n))}),r.forEach(o=>{o.classList.toggle("is-active",o.dataset.viewIndex===String(n))})}function v(e,n){let t=Number(e),r=Number(n);return!Number.isNaN(t)&&!Number.isNaN(r)?t-r:String(e).localeCompare(String(n))}function h(e,n,t){return e.length===0||(e.sort((r,o)=>{let c=r.children[n],s=o.children[n],i=c?.dataset?.value??c?.textContent??"",a=s?.dataset?.value??s?.textContent??"";return v(i,a)}),t==="desc"&&e.reverse()),e}function p(e,n,t){let r=e.querySelector("tbody");if(!r)return;let o=Array.from(r.querySelectorAll("tr"));if(t==="none"){let s=e._originalOrder;s&&s.forEach(i=>r.appendChild(i));return}if(e._originalOrder||(e._originalOrder=Array.from(o)),o.some(s=>s.classList.contains("bases-table-group-header"))){let s=[],i=null;for(let a of o)a.classList.contains("bases-table-group-header")?(i={header:a,rows:[]},s.push(i)):i?i.rows.push(a):(i={header:null,rows:[a]},s.push(i));for(let a of s)h(a.rows,n,t);for(let a of s)a.header&&r.appendChild(a.header),a.rows.forEach(l=>r.appendChild(l))}else h(o,n,t),o.forEach(s=>r.appendChild(s))}function g(e,n){e.querySelectorAll(".bases-table").forEach(r=>{let o=r.querySelectorAll("th[data-sortable='true']");o.forEach((c,s)=>{let i=()=>{let a=c.dataset.sortDirection||"none",l=a==="asc"?"desc":a==="desc"?"none":"asc";o.forEach(u=>{u!==c&&(u.dataset.sortDirection="none"),u.classList.remove("is-sorted-asc","is-sorted-desc")}),c.dataset.sortDirection=l,c.classList.toggle("is-sorted-asc",l==="asc"),c.classList.toggle("is-sorted-desc",l==="desc"),p(r,s,l)};c.addEventListener("click",i),n.push(()=>c.removeEventListener("click",i))})})}function b(){try{return decodeURIComponent((window.location.hash||"").replace(/^#/,"")).toLowerCase()}catch{return(window.location.hash||"").replace(/^#/,"").toLowerCase()}}function w(e){let n=b();if(!n)return-1;for(let t of e)if(t.dataset.viewSlug===n){let r=parseInt(t.dataset.viewIndex||"0",10);return Number.isNaN(r)?-1:r}return-1}function m(e,n){let t=e.querySelectorAll(".bases-view-tabs button");if(t.length===0)return;let r=parseInt(e.dataset.initialView||"0",10),o=w(t);d(e,o>=0?o:Number.isNaN(r)?0:r),t.forEach(s=>{let i=()=>{let a=parseInt(s.dataset.viewIndex||"0",10);d(e,Number.isNaN(a)?0:a);let l=s.dataset.viewSlug;l&&window.history.replaceState(null,"","#"+l)};s.addEventListener("click",i),n.push(()=>s.removeEventListener("click",i))});let c=()=>{let s=w(t);s>=0&&d(e,s)};window.addEventListener("hashchange",c),n.push(()=>window.removeEventListener("hashchange",c))}function f(){let e=document.querySelectorAll(".bases-page");if(e.length===0)return;let n=[];e.forEach(t=>{m(t,n),g(t,n)}),window.addCleanup&&window.addCleanup(()=>{n.forEach(t=>{t()})})}document.addEventListener("nav",()=>{f()});document.addEventListener("render",()=>{f()});f();
`;
